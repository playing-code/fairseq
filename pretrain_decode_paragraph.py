...
import utils_pretrain_decode_paragraph as utils
# import dgl
...
def train(cudaid, args,model,roberta_dict,rerank):
    ...
    mlm_data=utils.load_mask_data(os.path.join(args.data_dir,'data-bin-all/train' ),roberta_dict)
    #decode_data=utils.load_decode_data(os.path.join(args.data_dir,'data-bin-all/train'),roberta_dict)
    decode_data=mlm_data

    mlm_data_valid=utils.load_mask_data(os.path.join(args.data_dir,'data-bin-all/valid' ),roberta_dict)
    #decode_data_valid=utils.load_decode_data(os.path.join(args.data_dir,'data-bin-all/valid'),roberta_dict)
    decode_data_valid=mlm_data_valid
    ...
    #if cudaid==0:
    writer = SummaryWriter(os.path.join(args.data_dir, args.log_file+'_cuda_'+str(cudaid)) )
    epoch=0
    epoch_o=0
    model.train()
    # batch_t=52880-1
    ...
    if args.model_file !=None:
        ...
        if args.use_start_pos:
            ...
            start_pos=args.gpu_size*batch_t*2%(int(47675114/args.world_size)+1)
    ...
    #while True:
        ...
        data_batch=utils.get_batch(mlm_data,roberta_dict,args.gpu_size,decode_dataset=decode_data,rerank=rerank,dist=True,cudaid=cudaid,size=args.world_size,start_pos=start_pos)
        ...
        for  token_list, mask_label_list, decode_label_list in data_batch:
            ...
            if (batch_t)%accumulation_steps==0:
                ...
                if iteration%5000==0:
                    ...
                    accum_batch_loss_decode_valid=0
                    ...
                    model.eval()
                    with torch.no_grad():
                        for  token_list_valid, mask_label_list_valid, decode_label_list_valid in data_batch_valid:
                            #batch_t+=1
                            #assert candidate_id.shape[1]==2
                            # his_id=his_id.cuda(cudaid)
                            # candidate_id= candidate_id.cuda(cudaid)
                            ...
                            mask_label_list_valid=mask_label_list_valid.cuda(cudaid)
                            decode_label_list_valid=decode_label_list_valid.cuda(cudaid)

                            loss_mask_valid,sample_size_mask_valid,loss_decode_valid,sample_size_decode_valid=model(token_list_valid,mask_label_list_valid,decode_label_list_valid)

                            loss_mask_valid=loss_mask_valid/sample_size_mask_valid/math.log(2)
                            loss_decode_valid=loss_decode_valid/sample_size_decode_valid/math.log(2)
                            loss_valid=loss_mask_valid+loss_decode_valid

                            # print('loss: ',loss,' sample_size: ',sample_size)
                            # assert 1==0
                            
                            accum_batch_loss_valid+=float(loss_valid)
                            accum_batch_loss_mask_valid+=float(loss_mask_valid)
                            accum_batch_loss_decode_valid+=float(loss_decode_valid)
                    ...
                    accum_batch_loss_mask_t= accum_batch_loss_mask_valid/accumulation_steps_valid
                    accum_batch_loss_decode_t= accum_batch_loss_decode_valid/accumulation_steps_valid
                    ...
                    writer.add_scalar('Loss_mask/valid'+str(cudaid), accum_batch_loss_mask_t, step)
                    writer.add_scalar('Loss_decode/valid'+str(cudaid), accum_batch_loss_decode_t, step)
                    step+=1
                    torch.cuda.empty_cache()
                    model.train()
