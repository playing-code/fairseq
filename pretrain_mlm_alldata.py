...
import utils_pretrain_mlm_alldata as utils
# import dgl
...
def train(cudaid, args,model,roberta_dict,rerank):
    ...
    #while True:
        ...
        data_batch=utils.get_batch(mlm_data,roberta_dict,args.gpu_size,rerank=rerank,dist=True,cudaid=cudaid,size=args.world_size)
        for  token_list, mask_label_list in data_batch:
            batch_t+=1
            #assert candidate_id.shape[1]==2
            # his_id=his_id.cuda(cudaid)
            ...
            if (batch_t)%accumulation_steps==0:
                ...
                if iteration%5000==0:
                    data_batch_valid=utils.get_batch(mlm_data_valid,roberta_dict,args.valid_size,rerank=None,dist=True,cudaid=cudaid,size=args.world_size)
                    ...
                    with torch.no_grad():
                        for  token_list_valid, mask_label_list_valid in data_batch_valid:
                            #batch_t+=1
                            #assert candidate_id.shape[1]==2
                            # his_id=his_id.cuda(cudaid)
                            # candidate_id= candidate_id.cuda(cudaid)
                            ...
                            loss_mask_valid,sample_size_mask_valid=model(token_list_valid,mask_label_list_valid)

                            loss_mask_valid=loss_mask_valid/sample_size_mask_valid/math.log(2)
                            loss_valid=loss_mask_valid
