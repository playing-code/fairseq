...
import argparse
import os
from model_plain_bert_concat import  Plain_bert
from fairseq.models.roberta import RobertaModel
from utils_sample_concat import NewsIterator
from utils_sample_concat import cal_metric
import utils_sample_concat as utils
# import dgl
...
def train(model,optimizer,args):
    ...
    #while True:
        ...
        for  imp_index , user_index, candidate_id , label  in data_batch:
            batch_t+=1
            # if batch_t<=232240:
            # if batch_t<=317190:
            ...
            loss,sample_size=model(candidate_id, label)
            ...
            if (batch_t)%accumulation_steps==0:
                ...
                # if iteration%10==0:
                #     torch.save(model.state_dict(),'/Plain_bert_1024_concat'+str(epoch)+'.pkl')
                #writer.add_scalar('Loss/train', float(accum_batch_loss/accumulation_steps), iteration)
                #break
        torch.save(model.state_dict(), os.path.join(args.save_dir,'Plain_robert_concat'+str(epoch)+'.pkl'))
